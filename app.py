                ...
                if api_key:
                    with st.spinner("AI Analyst is evaluating context..."):
                        ...
                        {{
                          ...
                          "comparison_data": [
                            {{"Metric": "Metric Name", "Company": "Company Name", "Value": 150.0, "Timeline": "5-Year Outlook"}}
                          ]
                        }}
                        """
                        try:
...
def extract_tickers_via_llm(query, api_key):
    client = genai.Client(api_key=api_key)
    prompt = f"Identify all stock tickers or public company names mentioned in this user query. Return ONLY a JSON list of stock ticker symbols. If none are mentioned, return [].\n\nQUERY: {query}"
    try:
        response = client.models.generate_content(
...
def get_dynamic_company_context(ticker_symbol):
    try:
        ...
        return f"[DYNAMIC REPORT FOR {name} ({ticker_symbol})]\n{summary}\n{fin_text}"
    except Exception as e:
        return f"Could not retrieve dynamic financials: {str(e)}"
...
def get_company_context_for_rag(query, query_vector, api_key=None):
    ...
    if api_key and not detected_tickers:
        detected_tickers = extract_tickers_via_llm(query, api_key)
    ...
    for ticker in list(set(detected_tickers)):
        ...
        if is_baseline and chunks:
            ...
            for idx, c in enumerate(comp_chunks):
                ...
                context_parts.append(f"[Source: {comp_name_baseline} Report Segment {idx+1}]\n{txt}")
        else:
            context_parts.append(get_dynamic_company_context(ticker))
            retrieved_faiss_chunks.append({"company": ticker, "text": "Dynamic profile fetched."})
        ...
        with st.spinner("Fetching peer metrics..."):
            ...
            if not peer_data:
                ...
                st.dataframe(df_peers.style.format({
                    "Trailing P/E": "{:,.2f}", "Forward P/E": "{:,.2f}",
                    "Price / Sales": "{:,.2f}", "EV / EBITDA": "{:,.2f}"
                ...
                implied_df = pd.DataFrame([{"Multiple Method": method, "Implied Value": round(val, 2)} for method, val in implied_prices.items()])
                ...
                for idx, method in enumerate(y_methods):
                    ...
                    low_val, high_val = val * 0.85, val * 1.15
                    fig_football.add_trace(go.Bar(y=[method], x=[high_val - low_val], base=low_val, orientation='h', marker=dict(color='#58a6ff', opacity=0.8)))
